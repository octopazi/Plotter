...
- [x] **Config GUI — Extended Conversion Types:** Refactored the config dialog into a **3-tab QTabWidget** (File Format / Columns / Conversions) for a cleaner layout. The Conversions tab uses a **metadata-driven dynamic form** powered by `CONV_FIELD_SPECS` in `conversion_handlers.py` — the GUI reads field specs at runtime and builds widgets automatically. Adding a new conversion type only requires adding a handler + spec entry in `conversion_handlers.py`; **zero GUI code changes needed**. Includes Move Up/Down for sequential ordering. `conv_summary()` centralised in `conversion_handlers.py` is used by both the dialog and the Edit dialog for consistent display.
